...
var props := {
	...
	&"draw_reset": func (this: Mono) -> void:
		this.setp(&"draw_timer", this.item.get_time()),
	&"draw_to": func (this: Mono, draw_id: StringName) -> void:
		this.setp(&"cur_draw", draw_id),
	&"draw_reset_to": func (this: Mono, draw_id: StringName) -> void:
...
static func on_draw(this: Mono, item: SekaiItem) -> void:
	...
	match draw[0]:
		...
		&"fixed":
			...
			if t > timeout and await this.emitm(&"on_draw_loop"): TDraw.on_draw(this, item)
