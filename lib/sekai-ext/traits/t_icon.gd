...
var props := {
	...
	&"icon_draw": func (ctx: LisperContext, this: Mono, item: SekaiItem, rect: Rect2, pmodulate := Color(1, 1, 1, 1)) -> void:
	...
	&"icon_get_texture": func (ctx: LisperContext, this: Mono) -> Texture2D:
		var icon := this.getp(&"icon") as Array
		var texture = this.getp(&"asserts")[icon[0]]
		var clip = icon[1]
