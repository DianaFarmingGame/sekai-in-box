...
static func on_draw(ctx: LisperContext, this: Mono, ctrl: SekaiControl, _item) -> void:
	...
	match draw[0]:
		&"static":
			...
			if this.getp(&"draw_flip_h"):
				this.define._props[&"on_draw"] = func (ctx: LisperContext, this: Mono, ctrl: SekaiControl, item: SekaiItem) -> void:
