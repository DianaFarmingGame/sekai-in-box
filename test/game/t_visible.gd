	...
	&"draw": func (_sekai: Sekai, this, _dt, t: float) -> void:
		...
		match drawer[0]:
			...
			&"fixed":
				...
				var frame_idx := lerpf(0.0, (frames.size() as float), fmod(t - this.get_prop(&"draw_timer"), timeout) / timeout) as int
		...
		,
	&"reset_draw_timer": func (_sekai: Sekai, this, _dt, t: float) -> void:
		pass
}
